 ...
 */
caja *create_caja(distr e) {

    caja *b;

    /*Pide memoria para la estructura caja de la cola*/
    if ((b = (caja *) malloc(sizeof(caja))) == NULL) {
        errorMem(__LINE__);
        /*Si no hay memoria, retorna false*/
        return 0;
    }

    /*Asigna el elemento de la caja*/
    b->elem = e;
...
iterator create_iterator(queue q) {
    ...
    if ((it = (iterator) malloc(sizeof(caja*))) == NULL) {
        errorMem(__LINE__);
        return NULL;
    }
